...
export class CheckCommand {
  ...
  async run(projectPath: string, dbPath: string, args: string[], options: Options = {}): Promise<void> {
    ...
    const target = args[0];
    if (!target) {
      console.error("Error: Missing file path. Usage: omnigraph check <file-path> [--depth=N] [--json]");
      process.exit(1);
    }
    ...
    try {
      ...
      console.error(`Error running check: ${error instanceof Error ? error.message : 'Unknown error'}`);
      process.exit(1);
    }
  }
}
